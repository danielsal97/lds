...
StorageMinionMemory::StorageMinionMemory(size_t size_bytes)
    : m_data(size_bytes, 0), m_size(size_bytes)
{
}

void StorageMinionMemory::Read(size_t offset, std::vector<char>& buf)
{
...
void StorageMinionMemory::Write(size_t offset, const std::vector<char>& buf)
{
    if (offset + buf.size() > m_size)
    {
...
size_t StorageMinionMemory::GetDataSize(size_t offset) const
{
    std::shared_lock<std::shared_mutex> lock(m_lock);
